...
\textbf{Iterative Refinement.} Chain-of-thought prompting~\citep{wei2022chain}, self-debugging~\citep{chen2023program}, and reflexion show that iterative refinement can improve LLM outputs. However, recent work~\citep{huang2023large} reveals that self-refinement sometimes degrades solution quality without external feedback, raising questions about when iteration helps versus hurts---a gap our comparative evaluation addresses.

Our work provides the first systematic comparison of architectural paradigms (single-shot, iterative pipeline, multi-agent coordination) using controlled conditions to isolate the impact of architectural decisions on code correctness across task types and difficulty levels.
